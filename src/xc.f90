!> @brief This module contains exchange and correlation potentials
!> @details Currently supported:
!>   - LDA exchange (VWN: Vosko, Wilk & Nusair)
!>     - [S. H. Vosko, L. Wilk, and M. Nusair, Can. J. Phys. 58, 1200 (1980)](https://cdnsciencepub.com/doi/10.1139/p80-159) (doi: 10.1139/p80-159)
!>   - LDA correlation (PZ: Perdew & Zunger)
!>     - [J. P. Perdew and A. Zunger, Phys. Rev. B 23, 5048 (1981)](https://journals.aps.org/prb/abstract/10.1103/PhysRevB.23.5048) (doi: 10.1103/PhysRevB.23.5048)
module xc
...
implicit none
private
public get_Vxc_vwn, xc_pz, xc_pz2, xc_vwn2, xc_vwn3

contains

!> @brief This function calculates the LDA potential over a grid
!> @param[in] n Size of the charge density array
!> @param[in] rho(n) Charge density
!> @param[in] relat If T
subroutine xc_vwn3(n, rho, relat, c, exc, Vxc)
integer, intent(in) :: n ! size of the charge density array
real(dp), intent(in) :: rho(n) ! charge density
logical, intent(in) :: relat ! .true. return RLDA, otherwise LDA
real(dp), intent(in) :: c ! speed of light
real(dp), intent(out) :: Vxc(n), exc(n)
integer :: i
do i = 1, n
